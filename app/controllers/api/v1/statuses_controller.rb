...
class Api::V1::StatusesController < Api::BaseController
  ...
  before_action :set_automation_rule, only:  [:create]
  ...
  def set_automation_rule
    return if status_params[:automation_rule_id].blank?
    raise Mastodon::ValidationError if status_params[:scheduled_at].present?

    @automation_rule = current_account.conversation_automation_rules.active.find(status_params[:automation_rule_id])
    raise Mastodon::ValidationError if @automation_rule.choices.empty?
  end
