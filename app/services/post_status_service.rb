...
class PostStatusService < BaseService
  ...
  # @option [Enumerable<Account>] :hidden_mentions Accounts addressed without adding their handles to the stored text
  # @option [ConversationAutomationRule] :automation_rule Optional timeline automation template to snapshot
  # @return [Status]
  def call(account, options = {})
    ...
    @visible_text = @text
    @text = hidden_mention_text if @options[:hidden_mentions].present?
  ...
  def hidden_mention_text
    mentions = @options[:hidden_mentions].map { |account| "@#{account.acct}" }.join(' ')
    [mentions, @visible_text].compact_blank.join(' ')
  end
  ...
  def process_status!
    ...
    @status.text = @visible_text if @options[:hidden_mentions].present?
    ...
    ApplicationRecord.transaction do
      @status.save!
      attach_automation_card!
    end
  end

  def attach_automation_card!
    rule = @options[:automation_rule]
    return if rule.nil?

    raise Mastodon::ValidationError unless rule.account_id == @account.id && rule.active? && !@status.direct_visibility?

    card = @status.create_status_automation_card!(source_rule: rule, prompt: rule.prompt)
    rule.choices.each do |choice|
      card.choices.create!(label: choice.label, response_text: choice.response_text, image_url: choice.image_url, position: choice.position)
    end
  end
