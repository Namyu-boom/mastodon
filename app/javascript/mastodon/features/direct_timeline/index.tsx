...
const messages = defineMessages({
  ...
  allMessages: { id: 'messages.admin_all', defaultMessage: 'All conversations' },
  automation: { id: 'messages.automation', defaultMessage: 'Automatic replies' },
});
...
const DirectTimeline: React.FC<ColumnBase> = ({ columnId, multiColumn }) => {
  ...
  const canViewAllMessages = isAdministrator(permissions);
  ...
  return (
    ...
    >
          ...
          extraButtons={
            <>
              <ColumnHeaderButton
                showTextOnDesktop
                icon={RobotIcon}
                onClick={() => history.push('/conversations/automation')}
              >
                {intl.formatMessage(messages.automation)}
              </ColumnHeaderButton>
              {canViewAllMessages && (
                <ColumnHeaderButton
                  showTextOnDesktop
                  icon={ShieldCheckIcon}
                  onClick={() => history.push('/conversations/admin')}
                >
                  {intl.formatMessage(messages.allMessages)}
                </ColumnHeaderButton>
              )}
              <ColumnHeaderButton
                showTextOnDesktop
      ...
      {!isRedesignEnabled() && canViewAllMessages && (
        <button
          type='button'
          className='direct-timeline__admin-link'
          onClick={() => history.push('/conversations/admin')}
        >
      ...
      {!isRedesignEnabled() && (
        <button
          type='button'
          className='direct-timeline__automation-link'
          onClick={() => history.push('/conversations/automation')}
        >
          <RobotIcon size={18} />
          {intl.formatMessage(messages.automation)}
