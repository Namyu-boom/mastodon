...
RSpec.describe '/api/v1/statuses' do
  context 'with an oauth token' do
    ...
    describe 'POST /api/v1/statuses' do
      ...
      context 'with an automatic-response rule' do
        let(:rule) do
          user.account.conversation_automation_rules.create!(
            name: 'Menu',
            prompt: 'Choose one',
            choices_attributes: [{ label: 'Guide', response_text: 'Original response', position: 0 }]
          )
        end
        let(:params) { { status: 'Pick a guide', automation_rule_id: rule.id } }

        it 'stores an immutable public snapshot without exposing response text' do
          subject

          expect(response).to have_http_status(200)
          card = Status.find(response.parsed_body[:id]).status_automation_card
          expect(card).to have_attributes(prompt: 'Choose one', source_rule_id: rule.id)
          expect(card.choices.first).to have_attributes(label: 'Guide', response_text: 'Original response')
          expect(response.parsed_body[:automation_card][:choices].first).not_to have_key(:response_text)

          rule.choices.first.update!(response_text: 'Changed later')
          expect(card.choices.first.reload.response_text).to eq('Original response')
        end

        it 'rejects attaching the rule to a scheduled post' do
          params[:scheduled_at] = 1.day.from_now.iso8601

          subject

          expect(response).to have_http_status(422)
          expect(StatusAutomationCard.count).to eq(0)
        end
      end
